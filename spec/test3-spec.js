...
describe('scheduleMeetings', function() {
    ...
    it("should return an array of one room with one meeting when one meeting is scheduled",
        function() {
            var meeting1 = [
                new Date(2016, 5, 13, 18, 0, 0, 0),
                new Date(2016, 5, 13, 18, 30, 0, 0)
            ];
            ...
            expect(rooms[0]).toContain(meeting1);
        });

    it("should return an array of one room with two meeting " +
        "when two non colliding meetings are scheduled",
        function() {
            var meeting1 = [
                new Date(2016, 5, 13, 18, 0, 0, 0),
                new Date(2016, 5, 13, 18, 30, 0, 0)
            ...
            var meeting2 = [
                new Date(2016, 5, 13, 15, 0, 0, 0),
                new Date(2016, 5, 13, 15, 30, 0, 0)
            ];
            var rooms = scheduleMeetings([meeting1, meeting2]);
            expect(rooms.length).toBe(1);
            expect(rooms[0].length).toBe(2);
            expect(rooms[0]).toContain(meeting1);
            expect(rooms[0]).toContain(meeting2);
        });

    it("should return an array of two rooms with one meeting each" +
        "when two colliding meetings are scheduled, " +
        "the first starting earlier",
        function() {
            var meeting1 = [
                new Date(2016, 5, 13, 18, 0, 0, 0),
                new Date(2016, 5, 13, 19, 0, 0, 0)
            ];
            var meeting2 = [
                new Date(2016, 5, 13, 18, 30, 0, 0),
                new Date(2016, 5, 13, 19, 30, 0, 0)
            ];
            var rooms = scheduleMeetings([meeting1, meeting2]);
    ...
    it("should return an array of three rooms with one meeting each " +
        ...
        function() {
            ...
            expect(rooms[0].length).toBe(1);
            expect(rooms[0]).toContain(meeting);
            expect(rooms[1].length).toBe(1);
            expect(rooms[1]).toContain(meeting);
            expect(rooms[2].length).toBe(1);
            expect(rooms[2]).toContain(meeting);
        });
